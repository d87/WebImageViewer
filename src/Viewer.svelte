<script lang="ts">
	...
	let imagePath = playlist[index]
    let prefetchPath = playlist[index+1]
    let prefetchPath2 = playlist[index+2]
</script>
...
<div class="fixed bottom-0 left-0 top-0 right-0 z-10">
	...
	<!-- <HotKeys keyMap={keyMap} handlers={keybindHandlers} allowChanges={true}> -->
		...
		<div class="absolute top-0 left-0 flex justify-center items-center w-full h-full">
			...
			<div on:click={onPrev} class="fixed z-40 top-0 left-0 w-10 h-full bg-black opacity-0"></div>
			<div on:click={onNext} class="fixed z-40 top-0 right-0 w-10 h-full bg-black opacity-0"></div>
		</div>
		<!-- </FullScreen> -->
	<!-- </HotKeys> -->
</div>
