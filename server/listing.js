...
var natural_sort = require('alphanum-sort');
...
serveIndex.json = function _json (req, res, files, next, dir, showUp, icons, path) {
  ...
  stat(path, files, function (err, fileList) {
    ...
    const Dirs = fileList.filter((file) => {
      const isDir = file.stat && file.stat.isDirectory();
      return isDir
    }).map((file) => file.name)
    const sortedDirs = natural_sort(Dirs)

    const Files = fileList.filter((file) => {
      const isDir = file.stat && file.stat.isDirectory();
      return !isDir
    }).map((file) => file.name)
    const sortedFiles = natural_sort(Files)

    const body = JSON.stringify({ Dirs: sortedDirs, Files: sortedFiles })
    // serialize
